...
// SetQuota - assign a uniqueu project id to directory and set the quota limits
// for that project id
...
// getMaxProjectID - find the next project id to be used for containers
//
// Return 0 (and error) if project quota is not supported.
// Otherwise the first project quota to use is >=1.
// The first thing that getMaxProjectID() does is get the project id of the root directory.
// This test will fail if no pquota support.
// If pquota is supported and home directory does not have a project id the check will
// succeed and return value of 0 and then the next project id to use will be 1.
// There is also a 'hidden' feature here - project quota can be used to set a quota limit
// (using xfs_quota tool) on the storage driver directory, e.g.:
//    echo 999:/var/lib/docker/overlay2 >> /etc/projects
//    echo docker:999 >> /etc/projid
//
// In that case, the root project id will be used as a "start offset"
...
func getMaxProjectID(path string) (uint32, error) {
	...
	//
	maxProjectID, err := getProjectID(path)
	if err != nil {
		return 0, err
	}
	if maxProjectID < 1 {
	...
	for _, file := range files {
		...
		if maxProjectID <= projid {
			maxProjectID = projid + 1
		}
	}

	logrus.Debugf("getMaxProjectID(%s) = %d", path, maxProjectID)
	return maxProjectID, nil
}
