...
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (graphdriver.Driver, error) {
	...
	if backingFs == "xfs" {
		...
		if d.quotaCtl, err = graphdriver.NewQuotaCtl(home); err == nil {
			//
			// Test if file system supports project quotas by trying to set a quota
			// on a test directory
			//
			testdir = path.Join(home, "quotatest")
			if err := idtools.MkdirAllAs(testdir, 0700, rootUID, rootGID); err != nil && !os.IsExist(err) {
				return nil, err
			}

			var quota graphdriver.Quota
			quota.Size = 0
			if err := d.quotaCtl.SetQuota(testdir, quota); err == nil {
